...
\begin{enumerate}
  ...
  \color{blue}
  Thompson used direct NFA simulation because the alternative would mean he would need to convert the provided regular expression to an NFA, then a DFA. The conversion from NFA to DFA is an expensive one, from a time and space pespective. For small inputs, using direct NFA simulation makes more sense because it will likely have better time complexity than conversion to a DFA and then DFA simulation. For large inputs, the conversion from NFA to DFA could produce exponential numbers of states, which would use too much memory (poor space-complexity). 
